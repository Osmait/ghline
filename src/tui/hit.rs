...
impl<T: Copy> Region<T> {
    ...
    pub fn rows(target: T, area: Rect, row_h: u16, scroll: usize, len: usize) -> Self {
        Self {
            target,
            area,
            row_h: row_h.max(1),
    ...
    pub fn plain(target: T, area: Rect) -> Self {
...
mod tests {
    ...
    /// Any target will do here: none of this arithmetic looks at it.
    #[derive(Clone, Copy, PartialEq, Eq, Debug)]
    struct Somewhere;

    fn region(row_h: u16, scroll: usize, len: usize) -> Region<Somewhere> {
        Region::rows(Somewhere, area(), row_h, scroll, len)
    }
    ...
    fn an_empty_region_has_no_entry_anywhere() {
        ...
        assert_eq!(Region::plain(Somewhere, area()).index_at(5), None);
    }
    ...
    fn a_zero_row_height_cannot_divide_by_zero() {
        let r = Region::rows(Somewhere, area(), 0, 0, 50);
        assert_eq!(r.index_at(6), Some(1));
    }
}
