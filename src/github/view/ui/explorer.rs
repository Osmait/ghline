...
fn draw_tree(buf: &mut Buffer, area: Rect, app: &mut App) {
    ...
    let icon_style = crate::shared::config::file_icons();
    let entries: Vec<crate::github::data::TreeEntry> = app.fs_rows().into_iter().cloned().collect();
    ...
    for (row, i) in (app.fs_scroll..entries.len()).enumerate() {
        ...
        if !icon.is_empty() {
            let icolor = if e.is_dir {
                ...
                theme::lang(crate::shared::icons::language(&e.path))
            };
...
fn draw_file(buf: &mut Buffer, area: Rect, app: &mut App) {
    ...
    for (n, line) in source.iter().enumerate() {
        for (i, (from, to)) in crate::shared::syntax::wrap_ranges(line, width.max(1))
            .into_iter()
            .enumerate()
        {
