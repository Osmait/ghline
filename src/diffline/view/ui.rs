...
use crate::diffline::app::{App, FinderTab, Modal, Pane};
use crate::diffline::hit::{Region, Target};
use crate::diffline::model::{Kind, State};
...
fn tree(buf: &mut Buffer, area: Rect, app: &mut App) {
    ...
    for (i, f) in app.files.iter().enumerate().skip(app.tree_scroll) {
        ...
        let status_fg = match f.status {
            crate::diffline::model::Status::Added => theme::green(),
            crate::diffline::model::Status::Deleted => theme::red(),
            _ => theme::cyan(),
        };
        put(
            buf,
...
fn status_bar(buf: &mut Buffer, area: Rect, app: &App) {
    ...
    let pending = {
        ...
        p.push_str(match app.pending {
            crate::diffline::app::Pending::Leader => "␣",
            crate::diffline::app::Pending::G => "g",
            crate::diffline::app::Pending::Z => "z",
            crate::diffline::app::Pending::Bracket(crate::shared::nav::Dir::Prev) => "[",
            crate::diffline::app::Pending::Bracket(crate::shared::nav::Dir::Next) => "]",
            crate::diffline::app::Pending::None => "",
        });
        p
    };
    let mut x = put(
