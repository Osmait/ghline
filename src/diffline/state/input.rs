...
use crate::diffline::model::{Comment, Kind, State};
use crate::diffline::service::{Request, Write};
...
impl App {
    ...
    pub fn hits(&self) -> Vec<Hit> {
        let items: Vec<Hit> = match self.finder_tab {
            ...
            FinderTab::Grep => self.rows_across(|r| {
                matches!(
                    r.kind,
                    crate::diffline::model::Kind::Added | crate::diffline::model::Kind::Deleted
                )
            }),
    ...
    fn rows_across(&self, keep: impl Fn(&crate::diffline::model::Row) -> bool) -> Vec<Hit> {
        let mut out = Vec::new();
