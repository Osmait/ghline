...
use process_error::Result as Res;
...
/// Chooses a named available backend, or the first one present.
///
/// Configuration stays with the application: this crate receives the name
/// rather than deciding where settings live. A name for something that is not
/// installed falls through, because selecting a backend that can only fail is
/// worse than selecting the one that is actually here.
pub fn select(want: &str) -> &'static dyn Multiplexer {
    all()
        .iter()
        .find(|mux| mux.name() == want && mux.available())
        .or_else(|| all().iter().find(|mux| mux.available()))
        .copied()
        .unwrap_or(all()[0])
}
...
mod tests {
    ...
    impl Fake {
        ...
        fn boom() -> process_error::Error {
            process_error::Error::Spawn {
                program: "fake",
                install: None,
    ...
    #[test]
    fn selection_returns_one_of_the_available_backends() {
        let m = select("");
