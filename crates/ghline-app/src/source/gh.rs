...
pub use crate::shared::error::{Error, Result as Res};
...
fn run(args: &[&str]) -> Res<String> {
    let out = Command::new("gh")
        ...
        .map_err(|source| Error::Spawn {
            program: "gh",
            install: Some("install the GitHub CLI"),
            source,
        })?;
...
fn parse_unified(raw: &str) -> Vec<(String, Vec<Hunk>)> {
    ...
    for line in raw.lines() {
        ...
        if line.starts_with("@@") {
            ...
            hunk = Some(Hunk {
                hdr: crate::shared::text::expand_tabs(line).into_owned(),
                lines: Vec::new(),
            });
            continue;
        }
        ...
        match chars.next() {
            ...
            Some(sign @ ('+' | '-' | ' ')) => h.lines.push((
                sign,
                crate::shared::text::expand_tabs(chars.as_str()).into_owned(),
            )),
...
pub fn file_content(repo: &str, path: &str) -> Res<String> {
    ...
    Ok(crate::shared::text::expand_tabs(&raw).into_owned())
}
