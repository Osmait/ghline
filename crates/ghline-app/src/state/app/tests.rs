...
fn p_opens_the_finder_on_repositories() {
    ...
    assert_eq!(app.finder_source, crate::data::Source::Repos);
    assert_eq!(
        app.finder_len(),
        app.repos().len(),
...
fn tab_walks_the_sources_and_keeps_the_query() {
    ...
    ch(&mut app, 'x');
    press(&mut app, Key::Tab);
    assert_eq!(app.finder_source, crate::data::Source::Issues);
    assert_eq!(app.finder_query, "x", "the same words, somewhere else");
    for _ in 0..3 {
        press(&mut app, Key::Tab);
    }
    assert_eq!(app.finder_source, crate::data::Source::Repos, "it wraps");
}
...
fn a_commit_search_waits_for_something_to_search_for() {
    ...
    app.open_finder();
    app.finder_source = crate::data::Source::Commits;
...
mod all_repos {
    ...
    fn an_answer_lands_on_the_row_it_is_about_not_the_first_with_that_number() {
        ...
        let mut a = crate::fixture::prs(0).remove(0);
        a.num = 14;
