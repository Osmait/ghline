...
pub fn put(buf: &mut Buffer, x: u16, y: u16, max_x: u16, text: &str, style: Style) -> u16 {
    ...
    for c in text.chars() {
        ...
        // the source-text crate. But this is the one place every string in
        // either program passes through on its way to a cell, and a control
    ...
    fn put_writes_a_row_without_reaching_the_allocator() {
        ...
        let n = allocations(|| {
            put(
                ...
                0,
                190,
                "crates/ghline-app/src/state/app/input.rs",
                Style::default(),
            );
        });
        assert_eq!(n, 0);
