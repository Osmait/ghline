...
pub struct Palette {
    ...
    pub gutter: Color,
    // accents
    /// Links, TypeScript, and ghline's modal accent.
    pub cyan: Color,
...
/// Supplied by the application before custom themes are first listed.
static CUSTOM_DIR: OnceLock<Option<std::path::PathBuf>> = OnceLock::new();
...
/// Supplies the directory custom themes are loaded from and written to.
///
/// The first value wins, matching the once-per-process theme cache. Keeping
/// the path outside this crate lets each application decide where its config
/// belongs without teaching the drawing toolkit an application name.
pub fn use_dir(path: Option<std::path::PathBuf>) -> bool {
    CUSTOM_DIR.set(path).is_ok()
}

/// Where a theme file goes: `<config>/themes/<name>.theme`.
pub fn dir() -> Option<std::path::PathBuf> {
    CUSTOM_DIR.get().cloned().flatten()
}
...
pub fn template(name: &str) -> (std::path::PathBuf, String) {
    ...
    out.push_str(&format!("# {name} — a theme for ghline and diffline\n"));
    out.push_str("#\n");
