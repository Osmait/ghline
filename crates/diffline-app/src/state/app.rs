...
use crate::shared::error::Failure;
use crate::shared::nav::Dir;

use crate::model::{Anchor, ChangedFile, Comment, Row, Scope, State};
...
pub struct App {
    ...
    pub spans: HashMap<String, Vec<Vec<crate::shared::syntax::Span>>>,
...
impl App {
    ...
    pub fn agent_choices(&self) -> Vec<(String, bool)> {
        let mut out: Vec<(String, bool)> = self
            ...
            .collect();
        out.extend(
            crate::shared::config::agent_kinds()
                .into_iter()
                .map(|k| (k, true)),
        );
        out
    ...
    pub fn refusal(&self, a: &Agent) -> Option<String> {
        ...
        if !crate::shared::mux::current().is_free(a) {
            return Some(format!(
    ...
    fn worker_died(&mut self) {
        ...
        for c in &mut self.comments {
            if c.state == crate::model::State::Sending {
                c.state = crate::model::State::Queued;
            }
        }
...
mod tests {
    ...
    fn app() -> App {
        ...
        a.files = vec![ChangedFile {
            path: "src/a.rs".into(),
            status: crate::model::Status::Modified,
            add: 2,
            del: 1,
        }];
            ...
            Request::Refresh {
                ...
                files: Ok(vec![
                    ChangedFile {
                        ...
                        status: crate::model::Status::Added,
                        add: 1,
                        del: 0,
                    },
                    ChangedFile {
    ...
    fn a_failure_keeps_the_error_rather_than_a_sentence_about_it() {
        ...
        a.apply(Response::Files {
            scope: Scope::WorkingTree,
            result: Err(crate::shared::error::Error::Command {
                args: "git diff".into(),
                status: Some(1),
        ...
        a.apply(Response::Files {
            scope: Scope::WorkingTree,
            result: Err(crate::shared::error::Error::Spawn {
                program: "git",
                install: None,
    ...
    fn a_send_that_failed_puts_the_queue_back() {
        ...
        a.busy = true;
        a.apply(Response::Sent(Err(crate::shared::error::Error::Field {
