...
impl App {
    ...
    /// Moves through code rows one at a time.
    ///
    /// Passing `d` straight to `move_cursor` made it hunt in strides of `d`,
    /// so a `5j` that landed on a hunk header went five further — ten lines
    /// for one keystroke. The wheel shares this path so its cursor and the
    /// viewport cannot disagree on which part of the diff is being read.
    pub(crate) fn move_diff_by(&mut self, d: i64) {
        let step = d.signum();
        for _ in 0..d.abs() {
    ...
    pub fn hits(&self) -> Vec<Hit> {
        let items: Vec<Hit> = match self.finder_tab {
            ...
            FinderTab::Grep => self.rows_across(|r| {
                matches!(
                    r.kind,
                    crate::model::Kind::Added | crate::model::Kind::Deleted
                )
            }),
        ...
        crate::shared::fuzzy::rank(&self.query, &items, |h| h.label.as_str())
            .into_iter()
            .take(200)
    ...
    fn rows_across(&self, keep: impl Fn(&crate::model::Row) -> bool) -> Vec<Hit> {
        let mut out = Vec::new();
    ...
    pub fn run_command(&mut self, label: &str) -> bool {
        ...
        match label {
            ...
            "write a keymap to start from" => match crate::keys::path() {
                Some(path) => self.ask_write(Write::File {
                    path,
                    text: crate::keys::template(),
                }),
    ...
    pub fn palette_hits(&self) -> Vec<String> {
        ...
        crate::shared::fuzzy::rank(&self.query, &labels, |l| l)
            .into_iter()
...
mod tests {
    ...
    fn agent_at(
        kind: &str,
        status: crate::shared::mux::AgentStatus,
        focused: bool,
        cwd: &str,
    ) -> crate::shared::mux::Agent {
        crate::shared::mux::Agent {
            kind: kind.into(),
            pane: format!("w:{kind}"),
            cwd: cwd.into(),
    ...
    fn picking_a_running_agent_clears_a_pending_new_one() {
        let mut a = app();
        a.agents = vec![crate::shared::mux::Agent {
            kind: "claude".into(),
            pane: "w:1".into(),
            cwd: "/tmp/r".into(),
            status: crate::shared::mux::AgentStatus::Idle,
            title: String::new(),
            focused: false,
        }];
    ...
    fn a_rebound_key_does_the_new_thing_and_the_old_key_does_nothing() {
        let mut a = app();
        a.keys = crate::keys::Map::with("s = split\nj = none\n<C-n> = line-down\n");
        a.pane = Pane::Diff;
    ...
    fn unbinding_everything_behind_a_prefix_gives_the_prefix_back() {
        ...
        let mut a = app();
        a.keys = crate::keys::Map::with("]c = none\n]f = none\n]s = none\n] = line-down\n");
        a.pane = Pane::Diff;
        let before = a.cursor;
        press(&mut a, Key::Char(']'));
    ...
    fn a_busy_agent_is_refused_with_its_reason() {
        ...
        a.agents = vec![crate::shared::mux::Agent {
            kind: "claude".into(),
            status: crate::shared::mux::AgentStatus::Working,
            cwd: "/tmp/r".into(),
            pane: "wA:p1".into(),
            title: String::new(),
            focused: false,
