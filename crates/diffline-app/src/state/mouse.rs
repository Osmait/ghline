...
impl App {
    ...
    fn wheel(&mut self, at: Position, d: i64) {
        ...
        match hit.target {
            ...
            Target::Pane(Pane::Diff) => self.move_diff_by(d),
            _ => {}
        }
    }
}
...
mod tests {
    ...
    fn the_wheel_reaches_both_ends_of_a_diff() {
        ...
        assert_eq!(a.cursor, 19, "the cursor followed the wheel to the end");
        assert_eq!(a.diff_scroll, 20 - a.view_height);

        for _ in 0..10 {
            a.on_mouse(ev(Motion::ScrollUp, 40, 5));
            draw(&mut a);
        }
        assert_eq!(a.cursor, 0, "the cursor followed the wheel to the start");
