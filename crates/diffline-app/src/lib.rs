...
//!
//! The same shape ghline has, with the slow part being `git` and the
...
/// Application configuration and services shared with `ghline`.
pub use line_shared as shared;
/// Terminal drawing primitives shared with `ghline`.
pub use tui_kit as tui;
...
// The layers are how it is filed, not how it is spoken about: `diffline_app::app`
// reads better at a call site than `diffline_app::state::app`, and the layer is
