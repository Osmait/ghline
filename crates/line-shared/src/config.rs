...
pub fn apply_theme() {
    let _ = tui_kit::theme::use_dir(crate::settings::path_beside("themes"));
...
pub fn file_icons() -> crate::icons::Style {
    store()
        .get("file-icons")
        .map_or(crate::icons::Style::Nerd, |v| {
            crate::icons::Style::parse(&v)
        })
}
...
mod tests {
    ...
    fn a_missing_file_is_simply_no_settings() {
        assert!(crate::settings::parse("").is_empty());
    }

    #[test]
    fn comments_and_blank_lines_are_skipped() {
        let s = crate::settings::parse("# a note\n\n  \ntheme = mocha\n");
        assert_eq!(s.len(), 1);
    ...
    fn whitespace_around_the_equals_does_not_matter() {
        assert_eq!(crate::settings::parse("theme=mocha")["theme"], "mocha");
        assert_eq!(
            crate::settings::parse("  theme   =   mocha  ")["theme"],
            "mocha"
        );
    }
    ...
    fn a_line_with_no_equals_is_ignored_rather_than_fatal() {
        let s = crate::settings::parse("nonsense\ntheme = design\n");
        assert_eq!(s.len(), 1);
