...
/// The settings file: `$XDG_CONFIG_HOME/ghline/config`, or `~/.config`.
///
...
pub fn config_path() -> Option<PathBuf> {
    ...
    Some(base.join("ghline").join("config"))
}
...
fn write_all(p: &std::path::Path, all: &HashMap<String, String>) -> io::Result<()> {
    ...
    keys.sort();
    let mut out = String::from("# ghline. Written by the application; edit freely.\n");
    for k in keys {
...
mod tests {
    ...
    impl Tmp {
        fn new(tag: &str) -> Self {
            let p = std::env::temp_dir().join(format!("ghline-test-{}-{tag}", std::process::id()));
