...
//! the repository in front of it, and `--svg-live` for ghline, which
...
use crate::key::{Motion, Mouse, Press};
...
struct Sink {
    ...
    /// name and the crate builds both, so a diffline log headed `ghline`
    /// would be the first line of the file being wrong.
    program: &'static str,
    /// The flag that program replays a key sequence with. They differ —
    /// diffline draws the repository in front of it with `--svg`, ghline
